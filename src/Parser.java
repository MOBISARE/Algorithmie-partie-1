...
public class Parser {
    ...
    static Vector<Instruction> parse(String s2) {
        ...
        Vector<Instruction> v = new Vector<>();
        for (String instruction : instructions) {
            ...
            if (rhs.contains("*") || rhs.contains("+") || rhs.contains("-")) {
                ...
                if (terms[1].matches("[a-z]*")) t1 = new Variable(terms[1]);
                else t1 = new Entier(Integer.parseInt(terms[1]));
                Instruction t = new AssignOperator(lhs, operator, t0, t1);
                v.add(t);
            } else {
                Value t0;
                if (rhs.matches("[a-z]*")) t0 = new Variable(rhs);
                else t0 = new Entier(Integer.parseInt(rhs.trim()));
